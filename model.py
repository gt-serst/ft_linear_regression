...
def calculate_thetas_gradient(alpha, thetas_array, estimated_price, scaled_data, cleaned_data):
	...
	for (price, (x, y)) in zip(estimated_price, scaled_data):
		# The gradients are the partial derivatives of the Mean Squared Error (MSE) loss function with respect to each parameter: theta_0 is the intercept and theta_1 is the slope
		error = price - y
		gradient_theta_0 += error
		gradient_theta_1 += error * x
	epsilon = 1e-6
	# Stop the model when the partial derivatives with respect to each parameter are very close to zero (when the derivative curve is flat it means that a local/global minimum is found)
...
def unscale_thetas(thetas_array, cleaned_data):
	...
	# The formulas to unscale thetas are found by substituting the scaled x by the formulas used to scale it in the linear regression equation
